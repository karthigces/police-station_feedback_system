...
<form action="" method="GET">
	...
	<label id="printPageButton">ACK TYPE : </label> <select class="inputlength" name="ack_type" id="printPageButton">
	<option value=""></option>
...
<form action="" method="GET">
    ...
    if ( isset( $_GET['todate'] ) && !empty( $_GET['todate'] ) ){
		...
		$sql = "SELECT st_code,COUNT(DISTINCT sr_no) as TotalCount FROM salem_1c2af6696c4c67949752fa8006c4e63d ".$where." AND status=1 GROUP BY st_code ";
		$sql2 = "SELECT COUNT(sr_no) FROM salem_1c2af6696c4c67949752fa8006c4e63d ".$where." AND status=1"; 
		$result = $con->query($sql);
		echo "<center><h3>- - REPORT OF ".$_GET['sta_code']." POLICE STATION - ".$_GET['fromdate']." - ".$_GET['todate']."</h3></center>";
		...
		echo "<tr><td><b>TOTAL</b></td><td><b>$rows[0]</b></td></tr>";
	}
else
{
		$sql = "SELECT st_code,COUNT(DISTINCT sr_no) as TotalCount FROM salem_1c2af6696c4c67949752fa8006c4e63d WHERE status=1 GROUP BY st_code"; 
		$result = $con->query($sql);
		echo "<center><h3>- OVERALL REPORT -</h3></center>";
		echo "<table class='table table-bordered'>";
		echo "<tr align='center' class='info'>
		<th>Station Code</th>
		<th>Count</th></tr>";
		...
		$query = "SELECT COUNT(*) FROM salem_1c2af6696c4c67949752fa8006c4e63d WHERE status=1";
